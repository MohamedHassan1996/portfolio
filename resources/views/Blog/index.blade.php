            ...
            <div class="pt-8 pb-24">
                <div class="flex justify-start flex-col mt-32">
                    <div>
                        ...
                        <!-- blogs content -->
                        <div
                            class="blog-content-section flex justify-start items-start gap-[20px] flex-row box-border w-full mt-12">
                ...
                @endforeach
                                ...
                                @foreach ($blogs as $blog)
                                    <div
                                        ...
                                        <img src="{{ url("storage/$blog->thumbnail") }}"
                                            class="w-full h-[400px] rounded-lg" alt="image1" loading="lazy">
                                        <div
                                            class="bg-cover bg-no-repeat  box-border flex justify-start items-center flex-col pt-4">
                                            <div
                                                class="bg-[white] absolute top-[16px] right-[16px] box-border flex justify-start items-center flex-col w-[60px] py-2 rounded-lg">
                                                <p
                                                    class="[font-family:Montserrat,sans-serif] text-2xl font-semibold leading-[29px] text-[#333333]">
                                                    {{ $blog->created_at->format('d') }}</p>
                                                <p
                                        ...
                                        <div
                                            class="box-border flex justify-center items-stretch flex-col pt-11 pb-6 px-[23px]">
                                            <h2
                                                class="[font-family:Montserrat,sans-serif] text-[35px] font-semibold leading-[42px] text-[#333333] p-0 m-0 w-full sm:w-[600px] sm-max-w-[650px]">
                                                {{ $blog->title }}
                                            </h2>
                                            <p
                                                class="[font-family:Roboto,sans-serif] text-lg font-normal leading-[21.5px] text-[#777777] mt-[15.5px] m-0 p-0">
                                                {{ $blog->description }}
                                            </p>
                                            @if (app()->getLocale() == 'en')
                                                <a href="{{ url()->current() }}/{{ $blog->slug }}"
                                                    class="[font-family:Roboto,sans-serif] cursor-pointer text-lg font-normal text-[#166e1d] mt-3">
                                                    read me
                                                </a>
                                            @elseif(app()->getLocale() == 'ar')
                                                <a href="{{ url()->current() }}/{{ $blog->slug }}"
                            ...
                            <div
                                class="blogs-Categories border w-full md:w-[30%] bg-[white] box-border flex justify-start items-stretch flex-col gap-[31px] pt-[39px] px-[23px] rounded-lg border-solid border-[#e8e8e8]">
                                <div class="">
                                    <!-- Input Component is detected here -->
                                    <div
                                        class="border bg-[#fbfcf8] h-[50px] box-border flex flex-row items-center [justify-content:start] rounded-sm border-solid border-[#333333]">
                                ...
                                <div class="">
                                    ...
                                    <div class="w-[100.00%] box-border my-4">
                                        @foreach ($blogs as $blog)
                                            <div
                                                class="rounded bg-[rgba(51,51,51,0.04)] gap-4 box-border flex justify-start items-center flex-row w-[100.00%] h-[85px] pl-2 pr-[7px] first:mt-0 mt-[16.00px]">
                                                <img class="rounded h-[69px] max-w-[initial] object-cover w-[85px] box-border block border-[none]"
                ...
                <div class="pagination-container flex items-center justify-center space-x-2 mt-8 my-10">
                    ...
                    @foreach ($blogs->getUrlRange(1, $blogs->lastPage()) as $page => $url)
                        @if ($page == $blogs->currentPage())
                            <span
                                class="active bg-[#e8f1e8] [font-family:Montserrat,sans-serif] text-sm font-semibold leading-[17px] text-[#166e1d] inline-flex items-center justify-center h-8 box-border  px-[11px] py-0 rounded-sm border-[none]">{{ $page }}</span>
                        @else
                            <a href="{{ $url }}"
                                class="pagination-link text-sm font-semibold leading-[17px] text-[#333333]">{{ $page }}</a>
                        @endif
                    @endforeach
